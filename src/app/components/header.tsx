...
export function HeaderLink({text, link, isActive}:{text: string, link:string, isActive?: boolean}) {
   return(
    isActive ? 
  ...
  return(
    <>
      <div className='relative'>
        <button 
          className='hover:text-theme-red-500'
          onClick={toggle}>
          Serviços
        </button>
        <div className={`absolute top-8 z-30 w-[250px] flex flex-col gap-3 p-4 bg-theme-gray2-900 rounded ${transClass}`}>
          <HeaderLink link='/services/audit' text='Auditoria de Terceira Parte'/>
...
export function HeaderContainer({children}:{children:React.ReactNode}) {
  return(
    <header className='py-4 px-24 flex justify-center items-center bg-zinc-900 text-white'>
